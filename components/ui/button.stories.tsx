    ...
    return (
      <table className="border-separate border-spacing-x-4 border-spacing-y-2">
        <thead>
          <tr>
            ...
            {tamanos.map((t) => (
              <th
                key={t}
                className="text-faint text-left text-[11px] font-semibold tracking-[0.08em] uppercase"
              >
                {t}
              </th>
            ))}
        ...
        <tbody>
          {variantes.map((v) => (
            <tr key={v}>
              <td className="text-faint pr-2 text-[11px] font-semibold tracking-[0.08em] whitespace-nowrap uppercase">
                {v}
              </td>
              {tamanos.map((t) => (
                <td key={t}>
