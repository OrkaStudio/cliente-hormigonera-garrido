 ...
 */
export function Alerta({ titulo, children, tono = 'warn', accion, className }: AlertaProps) {
  const Icono = iconos[tono];
  return (
    <div
      className={cn(
        'border-line bg-card shadow-tarjeta relative overflow-hidden rounded-lg border',
        className,
      )}
    >
      ...
      >
        ...
        <div className="flex-1">
          ...
          {children ? (
            <div className="text-ink-soft mt-1 text-sm">{children}</div>
          ) : null}
        </div>
