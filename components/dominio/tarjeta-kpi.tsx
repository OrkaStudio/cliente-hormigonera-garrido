...
export function TarjetaKpi({
  ...
  return (
    <div
      className={cn(
        'border-hormigon-300 bg-card shadow-tarjeta flex flex-col rounded-lg border p-4',
        className,
      )}
    >
      <p className="text-hormigon-500 text-[11px] font-semibold tracking-[0.08em] uppercase">
        {rotulo}
      </p>
      <div className="mt-1.5">
        <Cifra valor={valor} unidad={unidad} tono={tono} tamano="xl" />
      </div>
      {pie ? <p className="text-hormigon-500 mt-1 text-xs">{pie}</p> : null}
      {extra ? (
        <div className="border-hormigon-200 mt-3 flex flex-wrap items-center gap-2 border-t pt-3">
          {extra}
        </div>
      ) : null}
    </div>
