...
export function EstadoVacio({ titulo, descripcion, accion, className }: EstadoVacioProps) {
  return (
    <div
      className={cn(
        'border-hormigon-300 flex flex-col items-center gap-2 rounded-lg border border-dashed px-6 py-12 text-center',
        className,
      )}
    >
      <p className="font-medium text-hormigon-700">{titulo}</p>
      {descripcion ? (
        <p className="text-hormigon-500 max-w-sm text-sm">{descripcion}</p>
      ) : null}
